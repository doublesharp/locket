...
- [ ] Local agent daemon (`docs/specs/agent.md`). `agent-socket-server`
  ...
  - [ ] **subtask** — agent-grant-table: SQLite-backed grant table
    ...
    `RequestGrant`/`ExpireGrant`/`RevokeGrant` handlers.
    **Critical path.**
...
- [ ] VS Code extension backed by the local agent
  ...
  - [ ] **subtask** — vscode-agent-client: TS client speaking the
...
- [ ] Sealed bundle. `bundle-container-format` shipped
  ...
  - [~] [c9077004] branch agent-c9077004/bundle-age-encryption, worktree .worktrees/agent-c9077004-bundle-age-encryption; **subtask** — bundle-age-encryption: integrate `age`/`rage` with multi-recipient support. **Critical path.**
...
- [~] Team command surfaces (`docs/specs/team-sync-recovery.md:5-110`).
  ...
  shipped. Remaining:
  - [ ] **subtask** — team-invite-create: signed invite with issuer
