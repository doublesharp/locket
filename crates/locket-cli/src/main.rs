...
pub(crate) use runtime::error::{
    ...
    profile_not_found_error, project_not_found_error, project_root_untrusted_error,
    scan_finding_blocked_error, secret_already_exists_error, secret_deleted_error,
    secret_not_found_error, secret_version_overflow_error, tty_required_error,
...
pub(crate) fn load_command_policy(
    ...
    policy_document
        ...
        .get(policy_name)
        .cloned()
        .ok_or_else(|| invalid_reference_error(format!("command policy not found: {policy_name}")))
}
